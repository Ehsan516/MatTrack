...
const Profile: React.FC<ProfileProps> = ({ role, isPremium, onUpgrade, profileData, onRefreshProfile, members, club, onClubAction }) => {
  ...
  const handleTransferOwnership = async () => {
    if (!selectedNewOwner || !club) return;
    setLoading(true);
    try {
      // Logic for transferring ownership: Update club owner_id and member roles
      await dataService.updateClub(club.id, { owner_id: selectedNewOwner });
      await dataService.updateMembershipRole(club.id, selectedNewOwner, 'OWNER');
      await dataService.updateMembershipRole(club.id, profileData.id, 'MEMBER');
  ...
  const handleDeleteClub = async () => {
    if (!passwordInput || !club) return;
    // Password verification logic would go here via Supabase Auth
    setLoading(true);
    try {
    ...
    <div className="space-y-6 pb-24 animate-in fade-in duration-500">
      ...
      <div className="bg-red-500/5 border border-red-500/20 rounded-[32px] overflow-hidden">
        ...
        <div className="divide-y divide-red-500/10">
          {isActualOwner && (
            <div onClick={() => setActiveModal('transfer')} className="flex items-center justify-between p-6 hover:bg-red-500/10 transition-colors cursor-pointer group">
          ...
          <div onClick={() => setActiveModal('delete_account')} className="flex items-center justify-between p-6 hover:bg-red-500/10 transition-colors cursor-pointer group">
           ...
           <div className="bg-slate-900 border border-red-500/30 w-full max-w-md p-8 rounded-[40px] shadow-2xl space-y-6">
              ...
              {isActualOwner && members.length > 1 ? (
                <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-2xl">
                   <p className="text-xs text-red-400 font-bold uppercase leading-relaxed text-center">Error: You must transfer academy ownership or delete your academy first because it has active members.</p>
                   <button onClick={() => setActiveModal(null)} className="w-full py-4 mt-4 bg-slate-800 rounded-xl font-black uppercase text-xs">Close</button>
                </div>
              ) : (
                <>
                  ...
                  <div className="flex gap-3">
                    <button onClick={() => setActiveModal(null)} className="flex-1 py-4 bg-slate-800 rounded-2xl font-black uppercase text-xs">Cancel</button>
                    <button className="flex-1 py-4 bg-red-600 text-white rounded-2xl font-black uppercase text-xs">Delete Forever</button>
                  </div>
                </>
              )}
           </div>
