    ...
    <div className="min-h-screen bg-slate-950 pb-24 text-slate-100 flex flex-col">
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 bg-slate-900/90 backdrop-blur-lg border-t border-slate-800 safe-bottom">
        <div className="flex justify-around items-center h-16 max-w-lg mx-auto px-4">
          ...
          <NavItem active={activeTab === 'members'} onClick={() => setActiveTab('members')} icon="users" label={role?.toString().toUpperCase() === 'OWNER' ? 'Roster' : 'Team'} />
          <NavItem active={activeTab === 'schedule'} onClick={() => setActiveTab('schedule')} icon="calendar" label="Classes" />
          <NavItem active={activeTab === 'journal'} onClick={() => setActiveTab('journal')} icon="book" label={role === UserRole.OWNER ? 'Insights' : 'Journal'} isPremiumIndicator={!isPremium} />
...
const NavItem: React.FC<{active: boolean, onClick: () => void, icon: string, label: string, isPremiumIndicator?: boolean}> = ({ active, onClick, icon, label, isPremiumIndicator }) => {
  const icons: any = {
    ...
    book: <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>,
